...
mod set1_test {
    ...
    fn single_char_file() {
        ...
        let result = lines
            .iter()
            .filter_map(|line| {
                String::from_utf8(xor_with_one_bytes(&hex_to_bytes(line), &target_key)).ok()
            })
            .collect::<Vec<String>>();
    ...
    fn test_decode_base64() {
        ...
        assert_eq!(
            String::from(input),
            String::from_utf8(decode_bytes).unwrap()
        );
    }
    ...
    fn ecb_test() {
        ...
        assert_eq!(
            data,
            aes128_ecb_encrypt(key, &aes128_ecb_decrypt(key, &data).unwrap()).unwrap()
        )
    }
}
